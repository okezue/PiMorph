#!/usr/bin/env python3
"""Pair VE-strat nuclear (w2) and VE-cadherin (w4) TIFFs per site into one manifest row.

The original data/ve_strat/manifest.csv lists only the VE-cadherin file, which made the
legacy pipeline segment junction signal alone and find no contacts. Each site has two
single-channel 2048x2048 uint16 files whose names share the `_D01_s<N>_` token.
...
# Plate well (e.g. D01, E05), site index within the well, wavelength index (w2 nuclei, w4 VE-cadherin).
            ...
            rows.append(
                {
                    ...
                    "path_geometry": str(waves["4"].relative_to(root)),
                    "path_nuclei": str(waves["2"].relative_to(root)),
                    "path_junction": str(waves["4"].relative_to(root)),
