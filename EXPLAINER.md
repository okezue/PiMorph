...
## End-to-end PiMorph flow

Figure 1 connects acquisition, proposal evidence, constrained reconstruction, the physical cell complex, uncertainty, interface measurements and study evaluation in one view. The main example executes the classical route on reproducible synthetic images; the neural route and later analysis modules are shown alongside it so their inputs and scope remain explicit. Training and benchmarking are separate activities. Dynamics needs a time series, multimarker analysis needs additional channels, and volumetric extraction begins with a labelled volume.

![The complete PiMorph flow from channel acquisition to reconstruction, interface measurements, evaluation and optional analysis modules](docs/figures/methods_full_flow.png)

**Figure 1. The complete PiMorph flow and its analysis branches.** Read downward through acquisition and proposals (A–B), reconstruction and the embedded complex (C–D), measurements and uncertainty (F/E), exports and evaluation (G), and the analysis modules (H–K). Solid teal arrows follow the executed path; dashed purple arrows identify the training connection and optional neural/ensemble branches; gray connectors show downstream module inputs. **A**, conceptual BioRender tissue artwork and independently rendered membrane, nuclear and junction channels from the synthetic example. Geometry is required; nuclei and a separate junction channel are optional. Coordinates here are pixels, with physical units available only when calibration is established. **B**, alternative classical and trained U-Net proposal routes. The displayed maps are actual classical boundary and seed evidence. The neural architecture is schematic: three normalized image channels plus three presence flags feed six heads—boundary, signed distance, seed, vertex, gap and log scale. The log scale belongs to distance regression, not calibrated reconstruction confidence. The training swatch denotes supervision from exact synthetic labels and annotated or pseudo-labelled real tiles; training is separate from reconstruction. **C**, seeded, masked watershed followed by cleanup produces the main example's 42 cell faces. This demonstration uses `fill_gaps=True`; the general decoder can exclude gap and exterior evidence. External integer labels can instead enter the complex extractor directly. **D**, exact extraction retains faces, connected interfaces, multicellular vertices, cyclic incidence and oppositely oriented half-edges. The gap/exterior inset is a separate synthetic topology example. Closed-loop, chain and Euler checks establish representational consistency, not biological accuracy.

**E**, an executed optional branch perturbs decoder parameters and adds merge/split candidates. Deduplication leaves 12 hypotheses, all structurally valid; the three displayed candidates have 43, 42 and 40 cells. Bars show normalized energy weights, with temperature selected for effective sample size 4. Contact color encodes the corresponding finite-ensemble weight on contacts present in the highest-weight candidate, from dark purple at low weight through teal to yellow at high weight. That candidate has 43 cells, showing that lowest generated energy need not recover truth; these weights are uncalibrated sensitivity summaries. **F**, the 42-cell main reconstruction supplies an oriented sampling strip, its unfolded intensity heatmap and transverse mean/maximum profiles along the same interface. The dashed threshold is 38 simulated photon units; the heatmap saturates at 140. Occupancy, continuity, width and sidedness are interface measurements, distinct from segmentation gap faces. **G**, exports include labels, overlays, structural tables and provenance; table areas are pixel counts and `n` is the number of cell sides. The separate hCEC benchmark inset shows recorded Cellpose-SAM (gray) and tuned neural PiMorph v6 (teal) means over five held-out fields. These are neither scores for this classical illustration nor five independent biological replicates.

**H**, the reconstructed contact network is displayed as a centroid-based dual projection, alongside conceptual aligned-marker strips for co-occupancy and joint appearance analysis. **I**, a schematic T1 exchanges the contacting cell pair; time-series analysis separately links cells and checks division, extrusion, contact and gap events. **J**, schematic force balance and conductance networks denote model-based extensions. Tensions are relative; tension and pressure identifiability depends on the force-balance model, geometry and available curvature information. Transport outputs remain functional proxies without TEER or tracer validation. **K**, labelled volumes enter a separate 3-D extractor; lumen-facing interfaces can supply a curved monolayer representation. The 3-D views are conceptual, and current validation is synthetic. H–K are separate APIs/scripts, not automatic stages of `pimorph reconstruct`; marker strips, temporal transitions, forces, conductance and 3-D surfaces drawn here are illustrations rather than new experimental results.

[Editable SVG](docs/figures/methods_full_flow.svg) · [Publication PDF](docs/figures/methods_full_flow.pdf) · [Figure provenance](docs/figure_data/methods_full_flow_provenance.json). Reproduce the figure with [the composition script](scripts/figures_full_flow.py) and regenerate its finite ensemble with [the ensemble script](scripts/figures_full_flow_ensemble.py).
...
A third proposer wraps Cellpose (`pimorph.infer.cellpose_proposer.CellposeProposer`, cellpose 4 or a fine-tuned model). One network pass yields masks, a flow field and cell-probability logits; the masks become crack boundary evidence (thin seams between cells filled by a geometric rule), one seed per mask scored by mean cell probability, a signed distance (so the decoder's exterior exclusion applies), a background probability, tricellular corners and a tissue mask. The constrained decoder then emits the same cells as a valid complex, and the energy, moves, fields and posterior run unchanged. Because the flow field is stored, masks at other flow-error and cell-probability thresholds are recomputed without a second network pass; `cellpose_hypotheses` decodes a 3 x 3 threshold grid with the decoder's perturbations and pools every hypothesis into one posterior scored against the default-threshold maps. On the held-out splits the proposer reproduces the fine-tuned Cellpose masks within 0.005 of every metric (hCEC adjacency F1 0.936 against 0.937, vertex F1 0.691 against 0.695; alizarine and FlyWing likewise) and, with nucleus-consistency merges, improves the RPE tiles (vertex F1 0.347 against 0.336, PQ 0.621 against 0.612, 150 cells against 168 with 140 true). The threshold posterior's MAP equals the default decode on hCEC, FlyWing and RPE (the energy does not prefer another threshold), at 30 to 35 times the cost; what it adds is the hypothesis family behind contact probabilities and credible intervals when Cellpose is the segmenter. Nucleus merges hurt on hCEC with Cellpose proposals (adjacency 0.892 against 0.936) because those masks are already well calibrated in cell count, so the merge is off by default for this proposer.

![A complete synthetic PiMorph reconstruction and molecular profile](docs/figures/methods_in_action.png)

**Figure 4. PiMorph in action with known synthetic input.** A, independent membrane and nuclear channels. B, `ClassicalProposer` boundary evidence and seeds. C, the constrained decoder's 42 cell faces, interfaces and vertices. The amber interface is measured in D–F. D, independently rendered junction signal and oriented sampling strip. E, those samples unwrapped by arclength and signed lateral offset, positive toward the forward half-edge's left face; display saturates at 140 photon units. F, transverse mean/maximum signal and an illustrative fixed threshold of 38 photon units. Amber spans have no sample above threshold: the measured unoccupied fraction and longest occupied-run fraction are both 0.30. Marker loss along a retained contact is distinct from a gap face. All dimensions are pixels; these are synthetic demonstration measurements, not biological estimates or held-out results. Representation checks pass. [Arrays and parameters](docs/figure_data/methods_in_action_provenance.json) record tissue seed 47 and acquisition seed 82.
...
$$w_h=\frac{\exp(-E_h/T)}{\sum_j\exp(-E_j/T)},\qquad \mathrm{ESS}=\frac{1}{\sum_h w_h^2}.$$
...
None of these sets is the target domain itself. Three systematic searches (the last on 2026-09-19, [docs/DATASET_HUNT_2026-09-19.md](docs/DATASET_HUNT_2026-09-19.md): 98 BioStudies accessions, 95 Zenodo dataset records, every IDR screen and project, the BioImage Model Zoo, figshare, Dryad, OSF, Hugging Face, GitHub and the data statements of the junction-analysis literature) found no public confluent vascular or lymphatic endothelial monolayer imaged with a junction marker (VE-cadherin, PECAM-1, ZO-1, claudin-5) that ships expert two-dimensional instance masks. The closest object is COVER (Seeler et al. 2024, Zenodo 10549102 and 11353017): zebrafish dorsal aorta with Pecam1-EGFP junctions and 296 manually traced cells, but as closed three-dimensional contours wrapped around a 25 to 30 μm vessel, which would need the surface-unrolling of that package before PiMorph's planar metrics apply. En face mouse aorta PECAM/ERG stacks (S-BIAD1920) exist without curated masks, and the 495 real PECAM-1 HUVEC fields used here for pseudo-labels have none. hCEC therefore stands in for the target, and the labs whose methods sections describe hand-traced endothelial junctions (JAnaP waypointing in the Stroka lab, the 70 annotated CDH5 images of the Veschini lab, the 900 delineated HUVEC cells of Searson and Prince) hold the data that would close this gap.
...
The paired table above compares a fine-tuned PiMorph with a zero-shot Cellpose-SAM, so part of the hCEC margin could simply be training data. To remove that asymmetry, `cpsam` was fine-tuned with the official recipe (AdamW, learning rate 1e-5 with ten warm-up epochs, weight decay 0.1, batch 8, 256 px crops, 100 epochs, final weights, no model selection) on exactly the fields PiMorph's v4 to v6 saw: the confluent pool (hCEC 10 fields, alizarine 20, FlyWing 32, RPE 52 tiles; 114 images, 55,884 cells, 80 min on one H100), hCEC alone (10 fields, 45 min), and HAEC train (348 fields, 196,795 cells, capped at 2,000 crops per epoch, 120 min). Items with a partial ROI were cropped to the ROI box with exterior pixels set to the in-ROI median, because Cellpose has no ignore mask. Test-time thresholds were left at their defaults, so a Cellpose hyperparameter search could still move these numbers. Pairs were written by [export_cellpose_training.py](scripts/export_cellpose_training.py); training details, logs and weights are catalogued in [runs/cellpose_ft](runs/cellpose_ft/); the benchmark loads a fine-tuned model through `PIMORPH_CELLPOSE_MODEL`. The source of record is [symmetric_summary.csv](runs/frontier/symmetric_summary.csv), with per-image files in [runs/frontier](runs/frontier/).

| Test data | Units | Method | Adjacency F1 | Vertex F1 | Vertex precision / recall | PQ | Boundary F1 | Vertex localization (px) |
|---|---:|---|---:|---:|---:|---:|---:|---:|
| hCEC | 5 fields | Cellpose-SAM zero-shot | 0.830 | 0.635 | 0.721 / 0.581 | 0.790 | 0.880 | 1.17 |
| | | Cellpose-SAM fine-tuned, confluent pool | **0.937** | **0.695** | 0.691 / 0.700 | **0.862** | **0.924** | 1.40 |
| | | Cellpose-SAM fine-tuned, hCEC only | 0.937 | 0.696 | 0.691 / 0.701 | 0.862 | 0.924 | 1.40 |
| | | PiMorph v6_pool, tuned | 0.867 | 0.680 | 0.678 / 0.683 | 0.816 | 0.916 | 1.28 |
| | | PiMorph with the fine-tuned Cellpose as proposer | 0.936 | 0.691 | 0.686 / 0.697 | 0.861 | 0.924 | 1.40 |
| Alizarine | 10 fields | Cellpose-SAM fine-tuned, confluent pool | **0.992** | 0.991 | 0.986 / 0.996 | 0.903 | 1.000 | 1.00 |
| | | PiMorph v6_pool, tuned | 0.987 | **0.992** | 0.994 / 0.990 | **0.920** | 0.999 | 1.00 |
| FlyWing | 10 images | Cellpose-SAM fine-tuned, confluent pool | **0.966** | 0.852 | 0.837 / 0.868 | **0.793** | 0.993 | 1.53 |
| | | PiMorph v6_pool, tuned | 0.911 | **0.875** | 0.869 / 0.881 | 0.780 | 0.994 | 1.59 |
| RPE | 20 tiles / 5 stacks | Cellpose-SAM fine-tuned, confluent pool | **0.681** | **0.336** | 0.301 / 0.383 | **0.612** | **0.751** | 1.70 |
| | | PiMorph v6_pool, tuned | 0.605 | 0.327 | 0.291 / 0.381 | 0.556 | 0.731 | 1.73 |
| | | PiMorph with the fine-tuned Cellpose as proposer, nucleus merges | 0.677 | **0.347** | 0.324 / 0.376 | **0.621** | 0.749 | 1.71 |
| HAEC | 86 fields | Cellpose-SAM fine-tuned, HAEC train | **0.555** | 0.331 | 0.371 / 0.304 | **0.648** | **0.862** | 1.46 |
| | | PiMorph v6_pool, tuned | 0.536 | **0.352** | 0.293 / 0.453 | 0.626 | 0.858 | 1.25 |
| hCEC | 5 fields | Cellpose-SAM fine-tuned on HAEC (cross-domain) | 0.207 | 0.098 | 0.258 / 0.077 | 0.264 | 0.525 | 2.05 |
| HAEC | 86 fields | Cellpose-SAM fine-tuned on the confluent pool (cross-domain) | 0.162 | 0.018 | 0.010 / 0.103 | 0.324 | 0.605 | 1.64 |

With matched training data the hCEC headline changes. Fine-tuned Cellpose-SAM leads adjacency F1 by 0.070 and PQ by 0.045 on all five fields, and vertex F1 by 0.015 on three of five, which is within the noise of a five-field test; PiMorph keeps the better vertex localization and a slightly closer vertex count. Ten hCEC fields are enough for Cellpose-SAM: the hCEC-only fine-tune equals the whole confluent pool on hCEC. On RPE the fine-tuned Cellpose-SAM leads every column. Alizarine is a tie. FlyWing keeps its split: PiMorph has the higher vertex F1 on all ten images, Cellpose-SAM the higher adjacency F1 on all ten. On HAEC, fine-tuning repairs the zero-shot Cellpose-SAM (vertex F1 0.039 to 0.331), yet PiMorph keeps the higher vertex F1 (ahead on 56 of 86 fields, through recall 0.45 against 0.30) and localization while Cellpose-SAM leads adjacency and PQ.

The one capability the symmetric table reserves for PiMorph is domain breadth: v6_pool is a single checkpoint that holds its accuracy on confluent and sub-confluent cultures at once, whereas each fine-tuned Cellpose-SAM collapses outside its own culture density (hCEC 0.098 for the HAEC model, HAEC 0.018 for the confluent model). The supported statement is therefore that PiMorph's proposal model is competitive with, not better than, a Cellpose-SAM fine-tuned on the same fields, and that the structural outputs of Sections 2 and 3 are what PiMorph adds to whichever segmenter scores higher on a few annotated fields of a new dataset. Every Cellpose-SAM row in this table already passes through PiMorph's extractor to obtain vertices, incident sets and gaps.
...
**Against an independent force readout.** The only public deposit found that pairs junction images with a mechanical measurement is Lang et al. 2019 (Zenodo 3257654, CC BY-NC-SA 4.0): 15 plasma nanoablations of parasegment-boundary actomyosin cables in E-cadherin:GFP Drosophila germband, with manual kymograph tracks of the two cut ends, from Scarpa et al. 2018. The pre-cut frame was reconstructed with v6_pool (39 to 79 cells per field), tensions were inferred in three settings, and the cut edge was identified from the reslice line (median distance 0.5 px). Results ([runs/mechanics_real](runs/mechanics_real/REPORT.md)): the cut edge is inferred above the field mean in 14 of 15 fields (sign test p 5e-4); edges on the cable line carry more tension than the other edges in 15 of 15 fields (+0.35 relative, Wilcoxon p 1.5e-4), reproducing the sign of the known twofold recoil of boundary cables; and across the 15 cuts the cut edge's relative tension ranks the recoil velocity with Spearman 0.64 (p 0.011, bootstrap 95% CI 0.16 to 0.86) in the tension-only setting with ridge 0.1, but only 0.2 (not significant) once pressure and Laplace curvature rows are added. This is a partial validation of relative tensions for one junction type at n = 15, reported as the best of three settings; on these short, neurally traced edges the curvature rows hurt. Absolute scale and pressures remain unvalidated.

Two further real comparisons are consistency checks rather than validations. On the DLITE ZO-1 hiPSC colonies of Vasan et al. 2019 (4 series, 102 hand-traced frames), PiMorph and DLITE tensions on the same edges agree with Spearman 0.27 (n 652 edge-frames, CI 0.19 to 0.35), and PiMorph's frame-to-frame tension consistency (0.14 to 0.66) is below DLITE's (0.58 to 0.72), with only 4 to 8 interior edges per frame carrying a PiMorph tension on these small islands. On the TissueMiner pupal wing (15 frames, curated labels), the inferred stress axis lies within 3 degrees of the proximal-distal axis in every frame, and proximal-distal edges carry 1.5 times the tension of anterior-posterior edges at the start, relaxing to 1.2 times over 5.8 h in step with the database elongation (Spearman 0.98); the geometry alone accounts for about 0.45 of that anisotropy. No FRET tension-sensor, wing-disc ablation or CellFIT validation deposit with images was found.
...
The through-plane proxy sums paths; the in-plane proxy solves a graph-Laplacian Dirichlet problem between designated cell/gap nodes. The latter has adjoint edge sensitivities proportional to squared potential differences. Finite-difference and Rayleigh-monotonicity tests verify the resistor calculation, not the biological mapping. Cell-node conductances are a chosen model and do not constitute an anatomical reconstruction of all paracellular transport routes. No TEER or tracer-flux measurement in the S-BIAD1169 fields validates the weights or predicted permeability, and barrier reports carry `validated: false`. Two real comparisons were run ([runs/function_real](runs/function_real/REPORT.md)). Against measured transepithelial resistance of NIST/NEI iPSC-RPE wells (Schaub, Hotaling, Bharti et al. 2020; DOI 10.18434/T4/1503229; 20 wells with TER, 1,032 registered 256 px ZO-1 tiles with hand-corrected border masks from 10 of them), the predicted permeability index of the curated complexes correlates with TER at Spearman −0.52 (n = 10 wells, p = 0.13, 95% CI −0.92 to 0.22): the right sign, not significant, on a TER range (780 to 1,030 Ω cm²) comparable to the within-well spread. The same index from v6_pool reconstructions gives −0.09, because the decoder splits about 60% more cells than the masks and the effective conductance scales with interface count. Against the ECIS resistance and tracer permeability read from the figures of the S-BIAD1169 paper (Bromberger et al. 2024), the per-condition effective conductance shows no agreement (Spearman −0.03 and 0.20 over 13 conditions; two treatments predicted leakier as measured, one predicted tighter despite a 600 Ω drop). The flag stays `validated: false`; the RPE result says the proxy is worth testing at adequate power, not that it works.

That test was then run on the same deposit's Healthy-2 series ([runs/function_real/healthy2](runs/function_real/healthy2/REPORT.md)): live QBAM bright-field absorbance of healthy-donor iPSC-RPE in 36 Transwell wells (Control, Aphidicolin, HPI4), imaged weekly over weeks 3 to 8 with TER read the same day, 216 TER values from 96 to 1146 Ω. There is no junction stain, so only the structural part of the proxy is testable: cells were segmented with a Cellpose-SAM fine-tuned on the 1,032 AMD QBAM tiles (held-out AMD wells: mask AP50 0.741, PQ 0.692, adjacency F1 0.703, vertex F1 0.657, against 0.459 / 0.444 / 0.385 / 0.343 zero-shot), decoded into complexes through the Cellpose proposer, and the resistor model was evaluated with a constant conductance per interface on 84 stratified well-timepoints (168 tiles). The result is a clear negative. The permeability index correlates with TER at Spearman +0.52 (n = 66 well-timepoints with a complex, p = 6e-06, 95% CI 0.30 to 0.70; +0.74 when the 18 HPI4 well-timepoints in which the segmenter finds no cells are counted as empty), the wrong sign for a leak proxy, and it is indistinguishable from cell density (Spearman +0.52; partial correlation of the index given density and area CV −0.02). Within condition the association survives (partial +0.38, p = 0.002) only because weeks are pooled; within condition and week it is +0.01. Leave-one-well-out prediction of TER from density and area CV reaches R² 0.15 and falls to 0.14 when the permeability index is added. With a constant interface conductance the index is interface density, a monotone function of cell density, and denser cells are what a maturing, pigmenting RPE monolayer shows; the transport model contributes nothing that density does not already say, and the largest TER contrast in the series (HPI4 wells at about 110 Ω) is seen only through the absence of pigment, an absorbance signal rather than a structural one. The flag therefore remains `validated: false`. What has still not been tested is the junction-coverage term itself, which needs a junction stain and TER on the same wells; the Healthy-2 result rules out the structural term as a barrier predictor on its own. No public dataset pairs the two, because barrier readings are per-insert electrode measurements that papers report as condition means while image deposits carry no electrode data; [docs/PAIRED_TEER_JUNCTION_PROTOCOL.md](docs/PAIRED_TEER_JUNCTION_PROTOCOL.md) specifies the three-week, 48-insert experiment (HUVEC on Transwells, six barrier-moving conditions, TEER within five minutes of fixation, VE-cadherin and ZO-1 staining, blinded pre-registered analysis) that would test the coverage term with 80% power at a true correlation of 0.4.
...
The figures can be regenerated with [figures_methods.py](scripts/figures_methods.py) and [figures_results.py](scripts/figures_results.py). The complete flow figure uses [figures_full_flow.py](scripts/figures_full_flow.py), which composes the committed method arrays, BioRender crop and explicit conceptual geometry. [figures_full_flow_ensemble.py](scripts/figures_full_flow_ensemble.py) regenerates its optional finite ensemble from the in-action arrays and validates every candidate. To rebuild the synthetic inputs and full flow together, run the method script, ensemble script and composition script in that order. These steps require no checkpoint or GPU. The results script uses compact committed inputs by default; `--refresh-data` rebuilds them from archived run outputs and records source hashes. [PNG, editable SVG and PDF assets](docs/figures/) accompany the [source arrays, tables and provenance](docs/figure_data/). The molecular schematic also has a [BioRender editing link](https://app.biorender.com/illustrations/952520c72c842c3e36ac5b90?slideId=bd997741-5662-a8b5-cfed-5eac3296d676). Captions distinguish synthetic demonstrations, historical microscopy examples and current quantitative tests.
