...
def hypothesis_statistics(h: Hypothesis, junction: np.ndarray, threshold: Optional[float] = None) -> Dict[str, float]:
    ...
    return {
        ...
        "n_cells": float(h.cx.cell_faces.size),
        "n_cell_cell_edges": float(h.cx.cell_cell_edges().size),
        "mean_degree": float(np.mean([d for _, d in to_simple_graph(h.cx).degree()]))
        if h.cx.cell_faces.size
        else float("nan"),
    }
...
def posterior_statistics(
    ...
    per_h = [hypothesis_statistics(h, junction, threshold) for h in post.hypotheses]
